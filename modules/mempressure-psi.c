...
static void mempressure_psi_setting_cb  (GConfClient *const gcc, const guint id, GConfEntry *const entry, gpointer const data);
static void mempressure_psi_setting_init(void);
static void mempressure_psi_setting_quit(void);

/* ------------------------------------------------------------------------- *
 * MEMPRESSURE_PSI_PLUGIN
 * ------------------------------------------------------------------------- */

static void mempressure_psi_plugin_quit(void);
static bool mempressure_psi_plugin_init(void);
 ...
 */
static guint
mempressure_psi_iowatch_add(int fd, bool close_on_unref,
...
/* ========================================================================= *
 * MEMPRESSURE_PSI_STATUS
 * ========================================================================= */

/** Configuration */
static gint  mempressure_psi_window         = MCE_DEFAULT_MEMPRESSURE_PSI_WINDOW;
static gint  mempressure_psi_warning_stall  = MCE_DEFAULT_MEMPRESSURE_PSI_WARNING_STALL;
static char *mempressure_psi_warning_type   = NULL;
static gint  mempressure_psi_critical_stall = MCE_DEFAULT_MEMPRESSURE_PSI_CRITICAL_STALL;
static char *mempressure_psi_critical_type  = NULL;

/** Cached memory use level */
static memnotify_level_t mempressure_psi_level = MEMNOTIFY_LEVEL_UNKNOWN;
...
mempressure_psi_update_level(void)
{
    ...
    if( prev != mempressure_psi_level ) {
        mce_log(LL_INFO, "mempressure_psi_level: %s -> %s",
                ...
                memnotify_level_repr(mempressure_psi_level));

        datapipe_exec_full(&memnotify_level_pipe,
                           GINT_TO_POINTER(mempressure_psi_level));
    }
}
...
mempressure_psi_init(void)
{
    ...
    mempressure_psi_warn_event_id =
        mempressure_psi_iowatch_add(mempressure_psi_warning_fd, false, G_IO_PRI,
    ...
    mempressure_psi_crit_event_id =
        mempressure_psi_iowatch_add(mempressure_psi_critical_fd, false, G_IO_PRI,
...
static guint mempressure_psi_setting_window_id = 0;

/** GConf notification id for mempressure.warning.stall time */
static guint mempressure_psi_setting_warning_stall_id = 0;

/** GConf notification id for mempressure.warning.type */
static guint mempressure_psi_setting_warning_type_id = 0;

/** GConf notification id for mempressure.critical.stall time */
static guint mempressure_psi_setting_critical_stall_id = 0;

/** GConf notification id for mempressure.critical.type */
static guint mempressure_psi_setting_critical_type_id = 0;
 ...
 */
static void
mempressure_psi_setting_cb(GConfClient *const gcc, const guint id,
                           GConfEntry *const entry, gpointer const data)
{
    ...
    else if( id == mempressure_psi_setting_window_id ) {
        gint old = mempressure_psi_window;
        gint val = gconf_value_get_int(gcv);
        if( old != val ) {
            mce_log(LL_DEBUG, "mempressure.window: %d -> %d", old, val);
            mempressure_psi_window = val;
            mempressure_psi_update_thresholds();
        }
    }
    else if( id == mempressure_psi_setting_warning_stall_id ) {
        gint old = mempressure_psi_warning_stall;
        gint val = gconf_value_get_int(gcv);
        if( old != val ) {
            mce_log(LL_DEBUG, "mempressure.warning.stall: %d -> %d", old, val);
            mempressure_psi_warning_stall = val;
            mempressure_psi_update_thresholds();
        }
    }
    else if( id == mempressure_psi_setting_warning_type_id ) {
        char       *old = mempressure_psi_warning_type;
        const char *val = gconf_value_get_string(gcv);
        if( !mempressure_psi_streq(old, val) ) {
            mce_log(LL_DEBUG, "mempressure.warning.type: %s -> %s", old, val);
            g_free(mempressure_psi_warning_type);
            mempressure_psi_warning_type = g_strdup(val);
            mempressure_psi_update_thresholds();
        }
    }
    else if( id == mempressure_psi_setting_critical_stall_id ) {
        gint old = mempressure_psi_critical_stall;
        gint val = gconf_value_get_int(gcv);
        if( old != val ) {
            mce_log(LL_DEBUG, "mempressure.critical.stall: %d -> %d", old, val);
            mempressure_psi_critical_stall = val;
            mempressure_psi_update_thresholds();
        }
    }
    else if( id == mempressure_psi_setting_critical_type_id ) {
        char       *old = mempressure_psi_critical_type;
        const char *val = gconf_value_get_string(gcv);
        if( !mempressure_psi_streq(old, val) ) {
            mce_log(LL_DEBUG, "mempressure.critical.type: %s -> %s", old, val);
            g_free(mempressure_psi_critical_type);
            mempressure_psi_critical_type = g_strdup(val);
            mempressure_psi_update_thresholds();
        }
    }
    else {
...
static void mempressure_psi_setting_init(void)
{
    mce_setting_notifier_add(MCE_SETTING_MEMPRESSURE_PSI_PATH,
                             MCE_SETTING_MEMPRESSURE_PSI_WINDOW,
                             mempressure_psi_setting_cb,
                             &mempressure_psi_setting_window_id);

    mce_setting_get_int(MCE_SETTING_MEMPRESSURE_PSI_WINDOW,
                        &mempressure_psi_window);

    mce_setting_notifier_add(MCE_SETTING_MEMPRESSURE_PSI_WARNING_PATH,
                             MCE_SETTING_MEMPRESSURE_PSI_WARNING_STALL,
                             mempressure_psi_setting_cb,
                             &mempressure_psi_setting_warning_stall_id);

    mce_setting_get_int(MCE_SETTING_MEMPRESSURE_PSI_WARNING_STALL,
                        &mempressure_psi_warning_stall);

    mce_setting_notifier_add(MCE_SETTING_MEMPRESSURE_PSI_WARNING_PATH,
                             MCE_SETTING_MEMPRESSURE_PSI_WARNING_TYPE,
                             mempressure_psi_setting_cb,
                             &mempressure_psi_setting_warning_type_id);
    ...
    mce_setting_notifier_add(MCE_SETTING_MEMPRESSURE_PSI_CRITICAL_PATH,
                             ...
                             mempressure_psi_setting_cb,
                             &mempressure_psi_setting_critical_stall_id);

    mce_setting_get_int(MCE_SETTING_MEMPRESSURE_PSI_CRITICAL_STALL,
                        &mempressure_psi_critical_stall);

    mce_setting_notifier_add(MCE_SETTING_MEMPRESSURE_PSI_CRITICAL_PATH,
                             MCE_SETTING_MEMPRESSURE_PSI_CRITICAL_TYPE,
                             mempressure_psi_setting_cb,
                             &mempressure_psi_setting_critical_type_id);
...
static void mempressure_psi_setting_quit(void)
{
    mce_setting_notifier_remove(mempressure_psi_setting_window_id),
        mempressure_psi_setting_window_id = 0;

    mce_setting_notifier_remove(mempressure_psi_setting_warning_stall_id),
        mempressure_psi_setting_warning_stall_id = 0;
...
static void
mempressure_psi_plugin_quit(void)
{
    mempressure_psi_quit();
    mempressure_psi_setting_quit();
}

static bool
mempressure_psi_plugin_init(void)
{
    bool success = false;

    mempressure_psi_setting_init();
...
G_MODULE_EXPORT const gchar *g_module_check_init(GModule *module)
{
    ...
    /* Initialize */
    if( !mempressure_psi_plugin_init() ) {
